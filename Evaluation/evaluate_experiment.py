...
import Evaluation.generic_util as util
from Evaluation.pro_curve_util import compute_pro
from Evaluation.roc_curve_util import compute_classification_roc
from Evaluation.roc_curve_segmentation_util import compute_segmentation_roc
...
def calculate_au_pro_au_roc(gt_filenames,
    ...
    # Compute the area under the segmentation ROC curve.
    #au_roc = util.trapezoid(fpr, tpr, x_max=integration_limit)
    #au_roc /= integration_limit
    print(f"Pixel-level segmentation AU-ROC: {roc_score}")
    #print(f"Pixel-level segmentation AU-ROC: {au_roc}")
    ...
    return au_pro, au_roc, pro_curve, roc_curve, roc_score
...
def evaluate(args):
    ...
    anomaly_maps_dir = args[0]
    dataset_base_dir = args[1]
    output_dir = args[2]
    pro_integration_limit = args[3]
    evaluated_objects = args[4]
    ...
    for obj in evaluated_objects:
        ...
        au_pro, au_roc, pro_curve, roc_curve, roc_score = \
        ...
        evaluation_dict[obj]['segmentation_au_roc'] = roc_score
